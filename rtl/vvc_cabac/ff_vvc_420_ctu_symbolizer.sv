...
module ff_vvc_420_ctu_symbolizer #(
  ...
  assign cur_leaf_allowed = cur_chroma_q ?
    ...
    (((cur_w_q <= LUMA_MAX_LEAF_SIZE) && (cur_h_q <= LUMA_MAX_LEAF_SIZE)) ||
     ((cur_mtt_q != 3'd0) &&
      (((cur_w_q <= LUMA_BOUNDARY_BT_SIZE) && (cur_h_q <= LUMA_MAX_LEAF_SIZE)) ||
       ((cur_h_q <= LUMA_BOUNDARY_BT_SIZE) && (cur_w_q <= LUMA_MAX_LEAF_SIZE)))));
      ...
      case (state_q)
        ...
        ST_POP: begin
          if (stack_count_q == 6'd0) begin
            if (!chroma_started_q) begin
              stack_x[0] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_y[0] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_w[0] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_h[0] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_cqt[0] <= 3'd1;
              stack_mtt[0] <= 3'd0;
              stack_implicit_mtt[0] <= 3'd0;
              stack_chroma[0] <= 1'b1;
              stack_x[1] <= 16'd0;
              stack_y[1] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_w[1] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_h[1] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_cqt[1] <= 3'd1;
              stack_mtt[1] <= 3'd0;
              stack_implicit_mtt[1] <= 3'd0;
              stack_chroma[1] <= 1'b1;
              stack_x[2] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_y[2] <= 16'd0;
              stack_w[2] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_h[2] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_cqt[2] <= 3'd1;
              stack_mtt[2] <= 3'd0;
              stack_implicit_mtt[2] <= 3'd0;
              stack_chroma[2] <= 1'b1;
              stack_x[3] <= 16'd0;
              stack_y[3] <= 16'd0;
              stack_w[3] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_h[3] <= DUAL_TREE_CHROMA_CU_SIZE;
              stack_cqt[3] <= 3'd1;
              stack_mtt[3] <= 3'd0;
              stack_implicit_mtt[3] <= 3'd0;
              stack_chroma[3] <= 1'b1;
              stack_count_q <= 6'd4;
        ...
        ST_DISPATCH: begin
          if (!cur_intersects) begin
            state_q <= ST_POP;
          end else if (cur_chroma_q) begin
            // VTM TypeDef.h TREE_C: the current separate chroma tree path is
            // represented as one transform-only chroma CU per visible 32x32
            // luma region, so no chroma split syntax is signalled here.
            chroma_cbf_cb_q <= 1'b0;
            residual_step_q <= 4'd0;
            state_q <= (sps_cclm_enabled_flag && cur_chroma_cclm_allowed) ?
              ST_CHROMA_CCLM : ST_CHROMA_MODE;
