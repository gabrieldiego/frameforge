...
- Generated 1- or 2-frame VVC stream assembled from one internally generated SPS/PPS sequence header, a color-derived Filler Data NAL unit, and generated picture slice NAL units. The current input path accepts planar YUV 4:2:0, 4:2:2, and 4:4:4 at 8, 10, 12, or 16 bits up to 64x64, then normalizes samples into the current 8-bit 4:2:0 VVC syntax. VTM-backed decode validation is currently limited to VVC geometries up to 32x32 because the 64x64 generated TU-grid body still needs to become compliant VVC coding-tree entropy. The current decoded picture uses a small quantized luma ladder on the 8x8-coded path and generated 16x16/32x32 coding-tree bodies while the 64x64 generated TU-grid path remains internal-only.
...
- Rust VVC encoder input handling for planar YUV frame sequences up to 64x64 with 4:2:0, 4:2:2, or 4:4:4 chroma at 8, 10, 12, or 16 bits per sample, currently normalizing to the 8-bit 4:2:0 VVC coding path.
- RTL VVC encoder input handling is parameterized with `VISIBLE_WIDTH`, `VISIBLE_HEIGHT`, `SAMPLE_BITS`, and `CHROMA_FORMAT_IDC` for wider input frames, input buses, and chroma planes while emitting the same normalized VVC stream as software.
...
- RTL VVC generator that drains a parameterized planar YUV input stream up to 64x64, samples color, and emits a sequence header, color-derived filler NAL, quantized residual payloads, per-picture Annex-B start codes, VVC NAL headers, and NAL payload bytes to match the Rust VVC stream.
...
- Replace CABAC packets with a minimal arithmetic CABAC writer fed by the same coding-tree events.
