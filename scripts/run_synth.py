...
from codec_config import CodecConfig, add_codec_arg, codec_config_from_args
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="top encoder MAX_VISIBLE_WIDTH synthesis parameter",
    )
    parser.add_argument(
        "--max-visible-height",
        type=int,
        default=DEFAULT_SYNTH_MAX_VISIBLE_HEIGHT,
        help="top encoder MAX_VISIBLE_HEIGHT synthesis parameter",
    )
    parser.add_argument(
        "--support-palette-444",
        type=parse_bool_int,
...
def tb_make_value(dut: str, codec: CodecConfig, target: str) -> str:
    completed = subprocess.run(
        ["make", "-C", "tb", f"CODEC={codec.name}", f"DUT={dut}", target],
        check=True,
        text=True,
...
def tb_verilog_sources(dut: str, codec: CodecConfig) -> list[Path]:
    return [
        (Path("tb") / line).resolve()
        for line in tb_make_value(dut, codec, "print-verilog-sources").splitlines()
    ]


def tb_toplevel(dut: str, codec: CodecConfig) -> str:
    return tb_make_value(dut, codec, "print-toplevel").splitlines()[-1]
    ...
    script.write_text(
        "\n".join(
            [
                ...
                *[
                    f"read_verilog -sv {include_flags} {quote_path(source)}".strip()
...
def run_yosys_cell_report(
    ...
    script = out_dir / "cell_report.ys"
    log = out_dir / "cell_report.log"
    json_netlist = out_dir / f"{top}.json"
    script.write_text(
        "\n".join(
            [
                "# Generated by scripts/run_synth.py",
                "# Flattened mapped-cell area report for routine synthesis comparisons.",
                f"read_json {quote_path(json_netlist)}",
                f"hierarchy -top {top}",
                "flatten",
                "stat -tech xilinx",
                "",
    ...
    print(f"Cell report script: {script}")
    rc = run_logged_command(
        [yosys, "-s", str(script)],
        log,
        "Yosys flattened cell report",
        timeout_sec,
        memory_limit_mb,
        warn_after_sec,
    )
    print(f"Cell report log: {log}")
    if rc == 0:
        print_cell_report_summary(log)
    return rc


def print_cell_report_summary(log: Path) -> None:
    metrics = parse_yosys_xilinx_cell_stats(log)
    if not metrics:
        print("Flattened Xilinx cell summary was not found in the Yosys log.")
        return

    print("Flattened Xilinx cell estimate:")
    print("| Metric | Count |")
    print("|---|---:|")
    for name in XILINX_CELL_REPORT_METRICS:
        if name in metrics:
            print(f"| {name} | {metrics[name]} |")


def parse_yosys_xilinx_cell_stats(log: Path) -> dict[str, int]:
    metrics: dict[str, int] = {}
    tracked_cells = set(XILINX_CELL_REPORT_METRICS) - {"Cells", "Estimated LCs"}
    for raw_line in log.read_text(errors="replace").splitlines():
        stripped = raw_line.strip()
        if stripped.startswith("Number of cells:"):
            metrics["Cells"] = int(stripped.rsplit(maxsplit=1)[-1])
            continue
        if stripped.startswith("Estimated number of LCs:"):
            metrics["Estimated LCs"] = int(stripped.rsplit(maxsplit=1)[-1])
            continue
        parts = stripped.split()
        if len(parts) == 2 and parts[0] in tracked_cells:
