...
External decoder validation is partially wired. The `vvc-eos` command emits only a VVC EOS NAL unit, and `vvc-skeleton` uses placeholder RBSP payloads. The `vvc-toy-4x4-video` command assembles a tiny VTM-accepted stream for geometries up to 16x16 from internally scheduled sequence and picture NALs, a color-derived Filler Data NAL, and toy slice entropy syntax. Larger toy geometries currently validate software/RTL byte alignment only; they are an incremental input-drain and parameter-set path, not complete clean-room VVC picture syntax yet.
